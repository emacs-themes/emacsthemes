...
import {
  DEFAULT_QUALITY as DEFAULT_WEBP_QUALITY,
  convertPngToWebp,
  toWebpPath,
...
/**
 * Converts a generated PNG artifact to WebP and removes the source PNG.
 *
 * @param {string} sourcePath - Path to the PNG artifact generated by the screenshot flow.
 * @returns {Promise<string>} The path to the resulting WebP file.
 */
...
/**
 * Processes a single Emacs mode for a theme: generates init.el, captures a PNG screenshot,
 * converts the screenshot to WebP, and optionally generates and converts a preview image.
 *
...
async function processThemeMode(
  ...
  if (modeName === "emacs-lisp-mode") {
    ...
    const previewWebpPath = await finalizeGeneratedImage(previewPath);
    console.log(`  ✅ Successfully generated ${basename(previewWebpPath)}`);
  }

  const screenshotWebpPath = await finalizeGeneratedImage(imagePath);
  console.log(`  ✅ Successfully generated ${basename(screenshotWebpPath)}`);
 ...
 *
 * @param {string} themeImagesDir - Target directory where transient PNG screenshots will be stored before WebP conversion.
