 ...
 *
 * Responsibilities:
 * - Load and cache the generated theme search index.
 * - Keep URL query parameter state in sync with the current search.
 * - Filter visible cards and update result/no-result UI states.
 */
(function () {
  const themesIndexUrl = "{{THEMES_INDEX_URL}}";
  const themesIndexCacheKey = `emacsthemes:index:${themesIndexUrl}`;
  ...
  /**
   * Toggles the loading state of the search input while index data is fetched.
   *
  ...
  /**
   * Builds the in-memory lookup map used for O(1) searchable-text access by theme id.
   *
   * @param {Array<{id: string, searchable: string}>} indexEntries - Parsed index records.
   */
  function buildSearchMap(indexEntries) {
    searchableById.clear();
    indexEntries.forEach((entry) => {
      if (!entry || typeof entry.id !== "string" || typeof entry.searchable !== "string") {
        return;
      }
      searchableById.set(entry.id, entry.searchable);
    });
  }
   ...
   *
   * @returns {Promise<Array<{id: string, searchable: string}>>} Resolved search index entries.
  ...
  function filterThemes(query) {
    ...
    cardEntries.forEach((entry) => {
      const searchable = searchableById.get(entry.id) || "";
  ...
  /**
   * Initializes search behavior by loading index data, hydrating initial query state,
   * and wiring submit/history event listeners.
   *
