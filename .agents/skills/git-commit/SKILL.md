...
Steps: 1. Analyze all current changes before planning commits:
...
git diff --staged
    ...
    2. Scan all changes for secrets before staging anything:
       ...
       If any matches are found:
          ...
          - Check whether the file is listed in `.gitignore` — if not, recommend adding it
          - Do not stage or commit until the user confirms or the sensitive content is removed
    ...
    3. Stage the changes:
       ...
       If the working tree contains entangled changes:
          - Identify the distinct changes — list what logical modifications exist
          - Determine dependencies — which changes require others to be present
          - Create a commit plan — order commits to satisfy dependencies
          - Stage incrementally — use partial staging to isolate each change
          - Verify at each step — ensure the repository works after each commit
    ...
    Before writing the trailers, **identify the underlying model** by asking yourself: *"What model am I?"*
